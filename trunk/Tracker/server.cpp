...
int Cserver::run()
{
	...
	while (!g_sig_term)
	{
		...
		// TorrentPier end
			...
			read_db_files();
		// External peer
		else if (time() - m_read_db_expeers_time > m_config.m_read_db_expeers_interval)
			read_db_expeers();
...
std::string Cserver::insert_peer(const Ctracker_input& v, bool udp, t_user* user)
{
	...
	if (m_use_sql && user && file.fid)
	{
		...
		q.p(user->uid==file.tor_poster_id);                             // releaser tinyint(1) NOT NULL DEFAULT '0'
		...
		q.p(uploaded);                                                  // up_add bigint(20) unsigned NOT NULL default '0',
		q.p(downloaded);                                                // down_add bigint(20) unsigned NOT NULL default '0',
		q.p(xbt_error);                                                 // xbt_error varchar(200) DEFAULT NULL
		q.p( ul_16k_count*3 > ul_gdc_count*2 ? ul_gdc_16k : ul_gdc );   // ul_gdc bigint(20) unsigned NOT NULL DEFAULT '0',
		q.p(ul_gdc_count);                                              // ul_gdc_c mediumint(9) unsigned NOT NULL DEFAULT '0'
		q.p(ul_16k_count);                                              // ul_16k_c mediumint(9) unsigned NOT NULL DEFAULT '0'
		q.p(ul_eq_dl_count);                                            // ul_eq_dl mediumint(9) unsigned NOT NULL DEFAULT '0'
	...
	if (!m_files_users_updates_buffer.empty())
	{
		...
		try
		{
			m_database.query("insert into " + table_name(table_files_users)
				+ " (topic_id, peer_hash, user_id, ip, ipv6, port, uploaded, downloaded,"
				+ " seeder, releaser, complete_percent, speed_up, speed_down,"
				+ " update_time, up_add, down_add, "
				+ " xbt_error, ul_gdc, ul_gdc_c, ul_16k_c, ul_eq_dl) values "
	...
	if (!m_users_updates_buffer.empty())
	{
		...
		try
		{
			m_database.query("insert into " + table_name(table_users) + " (u_down_total, u_up_total, " + column_name(column_users_uid) + ", u_up_release, u_up_bonus, max_up_speed, max_down_speed) values "
				...
				+ "  u_up_speed_today = values(max_up_speed),"
				+ "  u_down_speed_today = values(max_down_speed)");
		}
		catch (Cdatabase::exception&)
		{
		}
...
{
	std::ostringstream os;
	os << "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01//EN\"><meta http-equiv=refresh content=60><title>File-Tracker</title>";
	int leechers = 0;
	int seeders = 0;
	int torrents = 0;
	int gold = 0;
	...
	BOOST_FOREACH(t_files::const_reference i, m_files)
	{
		if (i.second.dl_percent == 0)
			gold ++;
		if (i.second.dl_percent == 50)
			silver ++;
		leechers += i.second.leechers;
		seeders += i.second.seeders;
	...
	os << "<table><tr><td>leechers<td align=right>" << leechers
		...
		<< "<tr><td>gold torrents<td align=right>" << gold
		<< "<tr><td>silver torrents<td align=right>" << silver
		<< "<tr><td>"
	...
	os << "<tr><td>"
		...
		// exPeers
		<< "<tr><td>read db expeers time<td align=right>" << t - m_read_db_expeers_time << " / " << m_config.m_read_db_expeers_interval
		// end
...
std::string Cserver::column_name(int v) const
{
	...
	// VIP begin
	case column_users_vip_status:
		return m_config.m_column_users_vip_status;
	// VIP end
...
// External peer
void Cserver::read_db_expeers()
{
	m_read_db_expeers_time = time();
	Csql_query q(m_database);
	q = "select rpad(info_hash,20,' '), ip, port from expeers";
	Csql_result result = q.execute();
	for (Csql_row row; row = result.fetch_row(); )
	{
		if (file( row[0].s() ))
		{
		    t_file& file = m_files[ row[0].s() ];
		    t_peers::key_type peer_key = row[1].s() + row[2].s() ;
		    ...
		    t_peer& peer = file.peers[ peer_key ];
		    peer.host_ = htonl(row[1].i());
		    peer.port = htons(row[2].i());

		    peer.xbt_error_empty = "";
		    peer.left = e_seeder;
		    peer.mtime = time();
		}
	}
